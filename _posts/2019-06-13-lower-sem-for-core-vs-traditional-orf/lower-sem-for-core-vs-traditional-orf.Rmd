---
title: "Lower Standard Error for CORE than Traditional CBM-R"
description: |
...
We can see that the confidence interval - 36 WCPM - around any score on that line of growth is more than the expected growth for the entire year - 32 WCPM. 

This large confidence is problematic when CBM-R measures are used to monitor student progress and to help make instructional decisions.

## Conditional *SEM* of CORE
...
We compared our CORE *CSEM* results to the reported traditional *SEM* of observed WCPM for the following CBM-R systems: aimsweb Plus, DIBELS 8th Edition, easyCBM, and FastBridge.

```{r}

trad_sem <- tibble::tribble(
                           ~System,    ~Grade,  ~SEM,
              "DIBELS 8th Edition", "Grade 2",   8.6,
              "DIBELS 8th Edition", "Grade 3", 10.07,
              "DIBELS 8th Edition", "Grade 4", 10.86,
...
1. We compute the average *CSEM*. More precisely, we compute the mean of *CSEM*^2^ and then take the square root, and compare that estimate to the reference *SEM*s (see table above).
...
The average estimated *CSEM* was less than 8 WCPM for all grades, and `r round(core_csem %>% filter(Grade == "Grade 2") %>% pull(ave_csem), 2)` WCPM for Grade 2, `r round(core_csem %>% filter(Grade == "Grade 3") %>% pull(ave_csem), 2)` WCPM for Grade 3, and `r round(core_csem %>% filter(Grade == "Grade 4") %>% pull(ave_csem), 2)` WCPM for Grade 4.
...
In addition, across all grades, 84% of estimated CORE *CSE*s were less than the reference *SEM* of 8 WCPM. For Grade 2, `r pctless8[1]`% of estimated *CSE*s were less than the reference *SEM* of 8 WCPM, `r pctless8[2]`% were less in Grade 3, and `r pctless8[3]`% were less in Grade 4. These percentages are all above the 50% criterion we established *a priori*. 
...
This figure also shows the relation between the estimated WCPM score and the *SE*, which are postively correlated (Grade 2 *r* = `r core_csem %>% filter(Grade == "Grade 2") %>% pull(cor)`; Grade 3 *r* = `r core_csem %>% filter(Grade == "Grade 3") %>% pull(cor)`; Grade 4 *r* = `r core_csem %>% filter(Grade == "Grade 4") %>% pull(cor)`). 

ALSO ANALYZE THE CSEM FOR THE LOWER END OF THE DISTRIBUTION...

## Conclusion

These results together provide solid evidence that the CORE scale scores contain lower *CSEM* estimates compared to the *SEM* of traditional CBM-R WCPM scores. Lower *CSEM* estimates make CORE better suited for measuring CBM-R, both for screening and progress monitoring, as a more precise score will lead to more accurate instructional decisions.
